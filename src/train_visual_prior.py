...
parser.add_argument(
    ...
    help="Path to a full training checkpoint (.pth) or train_log directory to resume from.",
)
parser.add_argument(
    "--skip_nonfinite_loss", action=argparse.BooleanOptionalAction, default=False,
    help="Skip a batch if the scalar training loss is NaN/Inf instead of aborting training.",
)
parser.add_argument(
...
def resume_ckpt(path, strict=True):
    ckpt_path = path
    if os.path.isdir(ckpt_path):
        ckpt_path = os.path.join(ckpt_path, 'last.pth')
    print(f"\n---resuming from {ckpt_path}---\n")
    checkpoint = torch.load(ckpt_path, map_location='cpu')
    model.load_state_dict(checkpoint['model_state_dict'], strict=strict)
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
    globals()["epoch"] = int(checkpoint['epoch']) + 1
    globals()["losses"] = checkpoint.get('train_losses', [])
    globals()["test_losses"] = checkpoint.get('test_losses', [])
    globals()["lrs"] = checkpoint.get('lrs', [])
    print(f"Resuming at epoch {epoch} / {num_epochs}")
    print(f"Restored {len(losses)} train losses, {len(test_losses)} test losses, {len(lrs)} lr values")
    del checkpoint
...
# resume a run if set, otherwise load multisubject stage1 ckpt if set
if resume_from_ckpt is not None:
    resume_ckpt(resume_from_ckpt)
...
for epoch in progress_bar:
    ...
    skipped_nonfinite_train_batches = 0
    ...
    for train_i in range(num_iterations_per_epoch):
        ...
        with torch.cuda.amp.autocast(dtype=data_type):
            ...
            if skip_nonfinite_loss and not torch.isfinite(loss).all():
                (
                    fwd_percent_correct,
                    bwd_percent_correct,
                    recon_cossim,
                ...
                skipped_nonfinite_train_batches += 1
                optimizer.zero_grad(set_to_none=True)
                print(f"Skipping non-finite train loss at epoch {epoch}, train_i {train_i}")
                continue
    ...
    if local_rank==0:
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            ...
            logs = {"train/loss": np.mean(losses[-(train_i+1):]),
                ...
                "train/skipped_nonfinite": skipped_nonfinite_train_batches,
                }
